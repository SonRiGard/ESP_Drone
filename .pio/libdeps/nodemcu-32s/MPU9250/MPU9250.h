...
int32_t Accel_x_bias=0, Accel_y_bias=0, Accel_z_bias=0;
int32_t Gyro_x_bias=0, Gyro_y_bias=0, Gyro_z_bias=0;
...
class MPU9250_ {
    ...
    float magnetic_declination = 11.96;  // moscow, 12/3/2023
...
public:
    ...
    bool mysetup(const uint8_t addr, const MPU9250Setting& mpu_setting = MPU9250Setting(), WireType& w = Wire) {
        // addr should be valid for MPU
        if ((addr < MPU9250_DEFAULT_ADDRESS) || (addr > MPU9250_DEFAULT_ADDRESS + 7)) {
            Serial.print("I2C address 0x");
            Serial.print(addr, HEX);
...
private:
    ...
    void mpu9250_my_config (){
        ...
        write_byte(mpu_i2c_addr, MPU_CONFIG, 0x09);  // Auto select clock source to be PLL gyroscope reference if ready else
        delay(200);
    ...
    void magnetometer_my_config (){
        uint8_t Data;
		
        //0 (8-average, 15 Hz default, normal measurement)
        write_byte(HMC5883L_ADDRESS, REGISTER_CONFIG_A, 0x70);

        //0 (8-average, 15 Hz default, normal measurement) gain = 5; Lsb/gauss =390; MGl/LSB = 2.56 ; Outputdata Range (0xF800->0x07FF) 
        write_byte(HMC5883L_ADDRESS, REGISTER_CONFIG_B, 0xA0);

        //Continuous-Measurement Mode
        write_byte(HMC5883L_ADDRESS, REGISTER_MODE, 0x00);
    }
...
public:
    void init_IMU(void){
        ...
        // wake up device
        write_byte(MPU9250_ADDRESS, PWR_MGMT_1, 0x00);  // Write a one to bit 7 reset bit; toggle reset device
        delay(100);

        // Set clock source to be PLL with x-axis gyroscope reference, bits 2:0 = 001
	    write_byte(MPU9250_ADDRESS, PWR_MGMT_1, 0x01);  // Write a one to bit 7 reset bit; toggle reset device
        delay(100);
        ...
        write_byte(MPU9250_ADDRESS, SMPLRT_DIV, 7); 

        // config accelerometer
        uint8_t c;
        c = read_byte(MPU9250_ADDRESS, ACCEL_CONFIG);
        write_byte(MPU9250_ADDRESS, ACCEL_CONFIG, (c & 0xE7));//acc -> 2g
        write_byte(MPU9250_ADDRESS, ACCEL_CONFIG2,  0x05);


        write_byte(MPU9250_ADDRESS, GYRO_CONFIG, 0x00);

        write_byte(MPU9250_ADDRESS, USER_CTRL, 0x00);
        write_byte(MPU9250_ADDRESS, INT_ENABLE, 0x01);
...
void Calib_magnetometer()
{
	...
	uint8_t raw_data[6];
	int32_t mag_temp[3] = {Mag_x_raw,Mag_y_raw,Mag_z_raw};
	...
	for (i = 0; i < 1500;i++)
	{
		...
		//HAL_Delay(12);
		if (i==0){
			max_mag[0]=Mag_x_raw;min_mag[0]=Mag_x_raw;
			max_mag[1]=Mag_y_raw;min_mag[1]=Mag_y_raw;
			max_mag[2]=Mag_z_raw;min_mag[2]=Mag_z_raw;
		}
		else{
			if (Mag_x_raw>max_mag[0]) max_mag[0]=Mag_x_raw;
			if (Mag_y_raw>max_mag[1]) max_mag[1]=Mag_y_raw;
			if (Mag_z_raw>max_mag[2]) max_mag[2]=Mag_z_raw;
			if (Mag_x_raw<min_mag[0]) min_mag[0]=Mag_x_raw;
			if (Mag_y_raw<min_mag[1]) min_mag[1]=Mag_y_raw;
			if (Mag_z_raw<min_mag[2]) min_mag[2]=Mag_z_raw;
		}
		delay(10);
	}
    // Get hard iron correction
    bias[0] = (max_mag[0] + min_mag[0]) / 2;  // get average x mag bias in counts
    bias[1] = (max_mag[1] + min_mag[1]) / 2;  // get average y mag bias in counts
    bias[2] = (max_mag[2] + min_mag[2]) / 2;  // get average z mag bias in counts
...
public:
    ...
    void update_my_mag() {
        ...
        if (read_my_mag(mag_count)) {
            ...
            m[0] = (float)(mag_count[0] * 2.56 );  // get actual magnetometer value, this depends on scale being set
            m[1] = -(float)(mag_count[1] * 2.56);
            m[2] = (float)(mag_count[2] * 2.56);
...
private:
    ...
    bool read_my_mag(int16_t* destination) {
        uint8_t raw_data[6];
        read_bytes(HMC5883L_ADDRESS, X_MSB, 6, &raw_data[0]);
            destination[0] = ((int16_t)raw_data[4] << 8) | raw_data[5];  // Turn the MSB and LSB into a signed 16-bit value
            destination[1] = ((int16_t)raw_data[0] << 8) | raw_data[1];  // Data stored as little Endian
            destination[2] = ((int16_t)raw_data[2] << 8) | raw_data[3]; 
        // Serial.print("paro   ");
    ...
    void collect_my_mag_data_to(float* m_bias, float* m_scale) {
        if (b_verbose)
            Serial.println("Mag Calibration: Wave device in a figure eight until done!");
        delay(4000);
        ...
        int16_t mag_temp[3] = {0, 0, 0};
        for (uint16_t ii = 0; ii < sample_count; ii++) {
            read_my_mag(mag_temp);  // Read the mag data
            for (int jj = 0; jj < 3; jj++) {
                if (mag_temp[jj] > mag_max[jj]) mag_max[jj] = mag_temp[jj];
                if (mag_temp[jj] < mag_min[jj]) mag_min[jj] = mag_temp[jj];
            }
        ...
        float bias_resolution = get_mag_resolution(MAG_OUTPUT_BITS::M16BITS);
        m_bias[0] = (float)bias[0] * 2.56 / 1000;  // save mag biases in G for main program
        m_bias[1] = (float)bias[1] * 2.56 /1000;
        m_bias[2] = (float)bias[2] * 2.56 /1000;

        // Get soft iron correction estimate
        //*** multiplication by mag_bias_factory added in accordance with the following comment
        //*** https://github.com/kriswiner/MPU9250/issues/456#issue-836657973
        scale[0] = (float)(mag_max[0] - mag_min[0])  / 2;  // get average x axis max chord length in counts
        scale[1] = (float)(mag_max[1] - mag_min[1])  / 2;  // get average y axis max chord length in counts
        scale[2] = (float)(mag_max[2] - mag_min[2])  / 2;  // get average z axis max chord length in counts
